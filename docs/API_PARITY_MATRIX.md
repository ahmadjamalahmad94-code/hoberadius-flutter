...
| الكروت | `cards_*`, `cards_checker_v2.html`, `card_users.html`, `card_user_360.html` | `/api/v1/cards/*`, `/api/v1/hotspot/cards/*`, `/api/v1/card-users/*` | `CardsListScreen`, `CardCheckerScreen`, batch screens, `CardUsersScreen`, `CardUser360Screen`, `HotspotCardsPortalScreen` | موجود Flutter لمستخدمي الكروت، وتمت إضافة بوابة شراء الكروت العامة Native مرتبطة بعقد `/api/v1/hotspot/cards/*` | P1/P5 | F2/F9 |
...
| الجلسات | `sessions_list.html`, تقارير الجلسات | `/api/v1/sessions/online`, `/api/v1/accounting/*` | `SessionsListScreen` | مكتمل كعرض تشغيلي للمتصلين الحاليين مع الطرد المباشر وآخر جلسات المحاسبة من `/api/v1/accounting/sessions` | P1 | F4 |
...
| النسخ الاحتياطي | `backups.html`, `mt_backups.html` | `/api/v1/backups/*` | `BackupsScreen` | مكتمل كعرض تشغيلي للنسخ المحلية وحالة جوجل درايف الصادرة من الخادم، ويبقى Restore محصورًا بالويب المحمي | P2 | F7 |
...
| بوابة الكروت | `portal_card*`, `card_marketplace.html`, `card_pricing*` | `/api/v1/hotspot/cards/*`, `/api/v1/card-users/*` جزئيًا | `HotspotCardsPortalScreen`, `CardUsersScreen`, `CardUser360Screen` | تجربة بوابة الكرت المستقلة متاحة Native للشراء وعرض الكروت، وما زالت إدارة التسعير وسوق الكروت تحتاج تعميقًا إضافيًا | P5 | F16 |
...
- النسخ الاحتياطي: أصبح عقد `/api/v1/backups/status` يعيد حالة جوجل درايف بشكل صريح وآمن، وتعرض شاشة Flutter الحالة والرسالة من الخادم بدل نص ثابت.
